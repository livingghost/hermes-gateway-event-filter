...
_INACTIVITY_WARNING_PREFIXES = (
    "⚠️ No activity for ",
    "No activity for ",
)
_INACTIVITY_TIMEOUT_PREFIXES = (
    "⏱️ Agent inactive for ",
    "Agent inactive for ",
)
...
def _is_inactivity_timeout_notice(content: Any) -> bool:
    text = str(content or "").strip()
    if not text:
        return False

    # These notices are generated by the gateway inactivity watchdog.  Keep
    # matching narrow so quoted user/model text is not treated as gateway noise.
    if len(text) > 900 or not any(text.startswith(prefix) for prefix in _INACTIVITY_TIMEOUT_NOTICE_PREFIXES):
        return False

    if any(text.startswith(prefix) for prefix in _INACTIVITY_WARNING_PREFIXES):
        return (
            "If the agent does not respond soon" in text
            and "timed out" in text
            and "use /reset" in text
        )

    if any(text.startswith(prefix) for prefix in _INACTIVITY_TIMEOUT_PREFIXES):
        return (
            "no tool calls or API responses" in text
            and "agent.gateway_timeout" in text
            and "use /reset" in text
        )
